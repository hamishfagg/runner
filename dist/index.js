...
async function run(workflow, options) {
    ...
    if (workflow.components?.credentials) {
        credentials = (0, liquidless_1.renderObject)(workflow.components?.credentials, { env }, { delimiters: templateDelimiters });
    }
        ...
        if (((test.config?.continueOnFail ?? true) || (config?.continueOnFail ?? true)) && (previous && !previous.passed)) {
            ...
            try {
                ...
                if (step.http) {
                    ...
                    // Body
                    if (step.http.body) {
                        requestBody = await (0, files_1.tryFile)(step.http.body, { workflowPath: options?.path });
                    }
                    //  JSON
                    if (step.http.json) {
                    ...
                    if (step.http.formData) {
                        ...
                        for (const field in step.http.formData) {
                            ...
                            if (step.http.formData[field].file) {
                                const file = await fs_1.default.promises.readFile(node_path_1.default.join(node_path_1.default.dirname(options?.path || __dirname), step.http.formData[field].file));
                    ...
                    if (step.http.auth) {
                        const authHeader = await (0, auth_1.getAuthHeader)(step.http.auth, credentials);
                        if (authHeader) {
                            if (!step.http.headers)
                                step.http.headers = {};
                            step.http.headers['Authorization'] = authHeader;
                        }
                        clientCredentials = await (0, auth_1.getClientCertificate)(step.http.auth, credentials, { workflowPath: options?.path });
                    }
                    // Set Cookies
                    ...
                    const res = await (0, got_1.default)(step.http.url, {
                        agent: {
                            https: agent
                        },
                    ...
                    if (step.http.check) {
                        ...
                        if (step.http.check.headers) {
                            ...
                            for (const header in step.http.check.headers) {
                                stepResult.checks.headers[header] = (0, matcher_1.checkResult)(res.headers[header.toLowerCase()], step.http.check.headers[header]);
                            }
                        }
                        // Check body
                        if (step.http.check.body) {
                            stepResult.checks.body = (0, matcher_1.checkResult)(body.trim(), step.http.check.body);
                        }
                        // Check JSON
                        if (step.http.check.json) {
                            const json = JSON.parse(body);
                            stepResult.checks.json = (0, matcher_1.checkResult)(json, step.http.check.json);
                        }
                        // Check Schema
                        ...
                        if (step.http.check.jsonpath) {
                            ...
                            for (const path in step.http.check.jsonpath) {
                                ...
                                stepResult.checks.jsonpath[path] = (0, matcher_1.checkResult)(result[0], step.http.check.jsonpath[path]);
                            }
                        }
                        // Check XPath
                        if (step.http.check.xpath) {
                            ...
                            for (const path in step.http.check.xpath) {
                                ...
                                stepResult.checks.xpath[path] = (0, matcher_1.checkResult)(result.length > 0 ? result[0].firstChild.data : undefined, step.http.check.xpath[path]);
                            }
                        }
                        ...
                        if (step.http.check.selectors) {
                            ...
                            for (const selector in step.http.check.selectors) {
                                ...
                                stepResult.checks.selectors[selector] = (0, matcher_1.checkResult)(result, step.http.check.selectors[selector]);
                            }
                        }
                        // Check Cookies
                        if (step.http.check.cookies) {
                            ...
                            for (const cookie in step.http.check.cookies) {
                                ...
                                stepResult.checks.cookies[cookie] = (0, matcher_1.checkResult)(value, step.http.check.cookies[cookie]);
                            }
                        }
                        // Check captures
                        if (step.http.check.captures) {
                            ...
                            for (const capture in step.http.check.captures) {
                                stepResult.checks.captures[capture] = (0, matcher_1.checkResult)(captures[capture], step.http.check.captures[capture]);
                            }
                        }
                        // Check status
                        if (step.http.check.status) {
                            stepResult.checks.status = (0, matcher_1.checkResult)(res.statusCode, step.http.check.status);
                        }
                        // Check statusText
                        if (step.http.check.statusText) {
                            stepResult.checks.statusText = (0, matcher_1.checkResult)(res.statusMessage, step.http.check.statusText);
                        }
                        // Check whether request was redirected
                        if ('redirected' in step.http.check) {
                            stepResult.checks.redirected = (0, matcher_1.checkResult)(res.redirectUrls.length > 0, step.http.check.redirected);
                        }
                        // Check redirects
                        if (step.http.check.redirects) {
                            stepResult.checks.redirects = (0, matcher_1.checkResult)(res.redirectUrls, step.http.check.redirects);
                        }
                        // Check sha256
                        if (step.http.check.sha256) {
                            const hash = crypto_1.default.createHash('sha256').update(Buffer.from(responseData)).digest('hex');
                            stepResult.checks.sha256 = (0, matcher_1.checkResult)(hash, step.http.check.sha256);
                        }
                        // Check md5
                        if (step.http.check.md5) {
                            const hash = crypto_1.default.createHash('md5').update(Buffer.from(responseData)).digest('hex');
                            stepResult.checks.md5 = (0, matcher_1.checkResult)(hash, step.http.check.md5);
                        }
                        // Check Performance
                        if (step.http.check.performance) {
                            stepResult.checks.performance = {};
                            for (const metric in step.http.check.performance) {
                                stepResult.checks.performance[metric] = (0, matcher_1.checkResult)(res.timings.phases[metric], step.http.check.performance[metric]);
                            }
                        }
                        // Check SSL certs
                        if (step.http.check.ssl && sslCertificate) {
                            ...
                            if ('valid' in step.http.check.ssl) {
                                stepResult.checks.ssl.valid = (0, matcher_1.checkResult)(stepResult.response.ssl?.valid, step.http.check.ssl.valid);
                            }
                            if ('signed' in step.http.check.ssl) {
                                stepResult.checks.ssl.signed = (0, matcher_1.checkResult)(stepResult.response.ssl?.signed, step.http.check.ssl.signed);
                            }
                            if (step.http.check.ssl.daysUntilExpiration) {
                                stepResult.checks.ssl.daysUntilExpiration = (0, matcher_1.checkResult)(stepResult.response.ssl?.daysUntilExpiration, step.http.check.ssl.daysUntilExpiration);
                            }
                        }
                        // Check byte size
                        if (step.http.check.size) {
                            stepResult.checks.size = (0, matcher_1.checkResult)(responseData.byteLength, step.http.check.size);
                        }
                        // Check co2 emissions
                        if (step.http.check.co2) {
                            stepResult.checks.co2 = (0, matcher_1.checkResult)(stepResult.response.co2, step.http.check.co2);
                        }
                    }
                }
                if (step.grpc) {
                    ...
                    let tlsConfig;
                    if (step.grpc.auth) {
                        tlsConfig = await (0, auth_1.getTLSCertificate)(step.grpc.auth, credentials, { workflowPath: options?.path });
                    }
                    const request = {
                        proto: Array.isArray(step.grpc.proto)
                            ? step.grpc.proto.map(proto => node_path_1.default.join(node_path_1.default.dirname(options?.path || __dirname), proto))
                            : node_path_1.default.join(node_path_1.default.dirname(options?.path || __dirname), step.grpc.proto),
                        host: step.grpc.host,
                    ...
                    const { data, size } = await (0, cool_grpc_1.makeRequest)(step.grpc.proto, {
                        ...request,
                        tls: tlsConfig,
                        beforeRequest: (req) => {
                    ...
                    if (step.grpc.check) {
                        ...
                        if (step.grpc.check.json) {
                            stepResult.checks.json = (0, matcher_1.checkResult)(data, step.grpc.check.json);
                        }
                        // Check Schema
                        ...
                        if (step.grpc.check.jsonpath) {
                            ...
                            for (const path in step.grpc.check.jsonpath) {
                                ...
                                stepResult.checks.jsonpath[path] = (0, matcher_1.checkResult)(result[0], step.grpc.check.jsonpath[path]);
                            }
                        }
                        // Check captures
                        if (step.grpc.check.captures) {
                            ...
                            for (const capture in step.grpc.check.captures) {
                                stepResult.checks.captures[capture] = (0, matcher_1.checkResult)(captures[capture], step.grpc.check.captures[capture]);
                            }
                        }
                        // Check performance
                        if (step.grpc.check.performance) {
                            ...
                            if (step.grpc.check.performance.total) {
                                stepResult.checks.performance.total = (0, matcher_1.checkResult)(stepResult.response?.duration, step.grpc.check.performance.total);
                            }
                        }
                        // Check byte size
                        if (step.grpc.check.size) {
                            stepResult.checks.size = (0, matcher_1.checkResult)(size, step.grpc.check.size);
                        }
                        // Check co2 emissions
                        if (step.grpc.check.co2) {
                            stepResult.checks.co2 = (0, matcher_1.checkResult)(stepResult.response?.co2, step.grpc.check.co2);
                        }
                    }
                }
                stepResult.passed = (0, runner_1.didChecksPass)(stepResult);
            }
            catch (error) {
                stepResult.passed = false;
